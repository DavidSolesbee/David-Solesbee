...
import {
  queryForTenant,
  queryScalarForTenant,
  type TenantAccess,
} from "@/lib/db/dealership";
...
function query<T = Record<string, unknown>>(
  ...
  return queryForTenant<T>(access, sql, params);
}
function queryScalar<T = number>(
  ...
  return queryScalarForTenant<T>(access, sql, params);
}
...
export function dataAsOf(access: TenantAccess): string {
  return (
    queryScalar<string>(
      access,
...
export function getSalesModule(opts: {
  access: TenantAccess;
  canCost: boolean;
  canMargin: boolean;
}): SalesModule {
...
export function getPartsModule(opts: { access: TenantAccess; canCost: boolean; canMargin: boolean }): PartsModule {
  const { access } = opts;
  const p = query<{ rev: number; cost: number; lines: number; qty: number }>(access,
  ...
  const byYear = query<{ year: string; value: number }>(access,
  ...
  const topManufacturers = query<{ label: string; value: number }>(access,
  ...
  const topParts = query<{ label: string; value: number }>(access,
...
export function getServiceModule(opts: { access: TenantAccess; canTechnician: boolean }): ServiceModule {
  const { access } = opts;
  const laborRevenue =
    queryScalar<number>(access,
  ...
  const statusBreakdown = query<{ label: string; value: number }>(access,
  ...
  if (opts.canTechnician) {
    ...
    topTechnicians = query<{ first: string; last: string; value: number }>(access,
...
export function getCustomersModule(opts: { access: TenantAccess; canContacts: boolean }): CustomersModule {
  const { access } = opts;
  const counts = query<{ total: number; biz: number }>(access,
  ...
  const rows = query<{ customerId: number; name: string | null; revenue: number }>(access,
  ...
  const newByYear = query<{ year: string; value: number }>(access,
...
export function getInventoryModule(opts: { access: TenantAccess; canCost: boolean }): InventoryModule {
  const { access } = opts;
  const totals = query<{
    units: number;
    retail: number;
  ...
  const byCategory = query<{ label: string; units: number; retail: number }>(access,
  ...
  const byCondition = query<{ label: string; value: number }>(access,
  ...
  const asOf = dataAsOf(access);
  const agingBuckets = query<{ label: string; value: number }>(access,
  ...
  const topMakes = query<{ label: string; value: number }>(access,
...
export function getPaymentsModule(opts: { access: TenantAccess }): PaymentsModule {
  const { access } = opts;
  const t = query<{ total: number; count: number }>(access,
  ...
  const byMethod = query<{ label: string; value: number }>(access,
